...
function renderTable() {
  ...
  filtered.forEach((idx, pos) => {
    ...
    tr.addEventListener('click', () => {
      ...
      // copy button stays active always
    });
...
function addSelectedToQuote() {
  ...
  // If same supplier+description+part+price exists, bump qty
...
// For copy-text formats
function baseSupplierName(s) {
  if (!s) return '';
  const trimmed = s.trim();
  // Remove trailing "2025" if present
...
// Items-only text with "(Supplier price list)" suffix
...
// Items-only, but without (Supplier price list) – used for PO emails
function buildItemsNoSuffixForSupplier(supplierKey) {
  const group = quoteItems.filter(item => {
    const fw = firstWord(baseSupplierName(item.supplier)).toLowerCase();
    return fw === supplierKey.toLowerCase();
  });
...
// Labour + travel summary for Step 3
...
function buildGroupedEmailBlocks() {
  // Group by first word of base supplier
  const groups = new Map(); // key: firstWord(lower), value: {label, items[]}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Load shared = load Parts.csv from same origin
  ...
  // Copy quote (with total, with supplier price list)
  ...
  // Copy for Email PO (subject line + grouped by supplier)
  $('copyQuoteEmail').addEventListener('click', () => {
    if (!quoteItems.length) return;
    const blocks = buildGroupedEmailBlocks();
    const txt = blocks.join('\n\n'); // blank line between supplier groups
    copyText(txt);
  });

  // Clear quote & return to parts
  ...
  // Open email draft via mailto (first supplier group)
  $('btnEmailDraft').addEventListener('click', () => {
    ...
    // For mailto body, use only first supplier block
    const body = blocks[0];
  ...
  // Initial state for quote buttons
  renderQuoteTable();
});
